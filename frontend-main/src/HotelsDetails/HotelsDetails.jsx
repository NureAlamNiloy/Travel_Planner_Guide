...
export default function HotelsDetails() {
    ...
    useEffect(() => {
      ...
      fetch(`https://scriptsurgeons.pythonanywhere.com/api/hotels/${id}/?format=json`)
  ...
  // console.log("get hotel id",selectedHotel)
  ...
  return (
      ...
      <div className={`bg-center py-10 `} >
        ...
        <img className="max-h-[25vh] w-auto" src={hotel?.image2} alt="" />
        <img className="max-h-[25vh] w-auto" src={hotel?.image3} alt="" />
        <img className="max-h-[25vh] w-auto" src={hotel?.image4} alt="" />
        <img className="max-h-[25vh] w-auto" src={hotel?.image5} alt="" />
        </div>
        </div>
      </div>

      <div className="py-10 px-10 mx-auto">
          <h1 className="text-3xl font-semibold mb-4">{hotel.hotel_name}</h1>

          <p className="mb-2">{hotel.location}</p>

          <p className="text-lg  mb-4">{hotel.description}</p>

          <p className="text-2xl font-semibold  mb-4">${hotel.price} per night</p>
        <div className="p-6">
          <div className="reservation-form grid grid-cols-3">
            <button onClick={()=>handleReservation(hotel.id)} className="max-w-[40%] mt-3 bg-blue-700 hover:bg-blue-600 text-white py-2 rounded-full">Reserve Hotel</button>
          </div>
      </div>
      </div>
